...
def set_improved_deployment_coordinates(model, dep_rate):
    N = model.settings["N"]
    L = model.settings["L"]
...
def main():
    ...
    Uaverage = -float(np.mean(U_end[[81 - 1, 81 - 1], 2]))
    np.savetxt(
        os.path.join(OUT_DIR, "Scissor_Bridge_2_Strength_During_Deploy_Step_History.csv"),
        np.asarray(history), delimiter=",",
        header="step,total_load_N,max_DCR,max_moment_Nm,moment_capacity_Nm,all_checks_safe", comments="",
    )
    summary = [
        "Scissor_Bridge_2_Strength_During_Deploy",
    ...
    write_summary("Scissor_Bridge_2_Strength_During_Deploy_Summary.txt", summary)

    truss_stress = truss_strain * model.bar.E_vec
    save_figure(model.plots.Plot_Shape_Bar_Stress(truss_stress), os.path.join(OUT_DIR, "Scissor_Bridge_2_Strength_During_Deploy_Bar_Stress.png"))
    save_figure(model.plots.Plot_Shape_Bar_Failure(pass_yn), os.path.join(OUT_DIR, "Scissor_Bridge_2_Strength_During_Deploy_Bar_Failure.png"))
    save_figure(model.plots.Plot_Deformed_Shape(U_end), os.path.join(OUT_DIR, "Scissor_Bridge_2_Strength_During_Deploy_Deformed.png"))
