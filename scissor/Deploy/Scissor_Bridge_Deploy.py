...
from scissor_common import (
    ...
    standard_deploy_delta,
    standard_deploy_supports,
)
...
def main():
    ...
    N = 8
    model = build_scissor_model(variant="standard", analysis="deploy", N=N)
    if model.settings["N"] != N:
        raise RuntimeError(f"Scissor deploy must use N={N}; got N={model.settings['N']}.")
    model.assembly.Initialize_Assembly()

    save_geometry_diagnostics(model, out_dir, "Scissor_Bridge_Deploy")

    ta = Solver_NR_TrussAction()
    ta.assembly = model.assembly
    ta.supp = standard_deploy_supports(model.node.coordinates_mat.shape[0], model.settings["N"])
    ta.verbose = False

    source_steps = 2000
    analysis_steps = int(os.environ.get("SCISSOR_DEPLOY_STEPS", "120"))
    gif_stride = max(1, analysis_steps // int(os.environ.get("SCISSOR_GIF_FRAMES", "100")))
    ...
    for step in range(analysis_steps):
        ...
        dL = standard_deploy_delta(source_step)
        ta.increStep = 1
        ta.iterMax = 40
        ta.tol = 1e-4
    ...
    np.save(os.path.join(out_dir, "ScissorUhis.npy"), Uhis)

    model.plots.fileName = os.path.join(out_dir, "Scissor_Bridge_Deploy.gif")
    model.plots.Plot_Deformed_His(Uhis[::gif_stride, :, :])

    U_end = Uhis[-1, :, :]
    save_figure(model.plots.Plot_Deformed_Shape(U_end), os.path.join(out_dir, "Scissor_Bridge_Final.png"))
