...
def main():
    ...
    np.savetxt(
        os.path.join(OUT_DIR, "Kirigami_Truss_Load_To_Fail_Step_History.csv"),
        np.asarray(history), delimiter=",", header="step,total_load_N,max_DCR,all_members_safe", comments="",
    )
    summary = [
        "Kirigami_Truss_Load_To_Fail",
    ...
    write_summary("Kirigami_Truss_Load_To_Fail_Summary.txt", summary)

    truss_stress = truss_strain * bar.E_vec
    save_figure(plots.Plot_Shape_Bar_Stress(truss_stress), "Kirigami_Truss_Load_To_Fail_Bar_Stress.png")
    save_figure(plots.Plot_Shape_Bar_Failure(pass_yn), "Kirigami_Truss_Load_To_Fail_Bar_Failure.png")
    save_figure(plots.Plot_Deformed_Shape(U_end), "Kirigami_Truss_Load_To_Fail_Deformed.png")
