...
class CD_Elements_RotSprings_3N:
    ...
    def Solve_Theta(X):
        """
        X: (3,3) rows are Xi, Xj, Xk
        theta = acos( dot(v1,v2) ), v1 = Xi-Xj, v2 = Xk-Xj (normalized)
        """
        Xi = X[0, :]
        Xj = X[1, :]
        Xk = X[2, :]
    ...
    # -----------------------
    # local force/stiff (CD)
    # -----------------------
    def Solve_Local_Force(self, X, theta0, K):
        """
        Central difference gradient of potential wrt 9 dofs (3 nodes x 3 dims)
        Return Flocal: (9,)
        """
        delta = self.delta
        Flocal = np.zeros(9, dtype=float)

        # flatten dof mapping: i=0..8 -> node_idx=i//3, dim=i%3
        for i in range(9):
            node_idx = i // 3
            dim = i % 3
    ...
    def Solve_Local_Stiff(self, X, theta0, K):
        """
        Central difference Hessian using force function
        Return Klocal: (9,9)
        """
        delta = self.delta
    ...
    def Initialize(self, node):
        """
        Set theta_current_vec and theta_stress_free_vec from initial geometry
        """
    ...
    def Solve_Global_Force(self, node, U):
        """
        Return Trs: (3*nodeNum,) global internal force vector
        """
        if self.node_ijk_mat is None:
            raise ValueError("rotSpr3N.node_ijk_mat is not set.")
        ...
        for i in range(rotSprIJK.shape[0]):
            ...
            Flocal = self.Solve_Local_Force(X, theta0[i], rotSprK[i])  # (9,)

            # scatter to global (each node gets 3)
    ...
    def Solve_Global_Stiff(self, node, U):
        """
        Return Krs: (3*nodeNum, 3*nodeNum) global stiffness matrix
        """
        if self.node_ijk_mat is None:
            raise ValueError("rotSpr3N.node_ijk_mat is not set.")
        if self.theta_stress_free_vec is None:
        ...
        for i in range(rotSprIJK.shape[0]):
            ...
            Klocal = self.Solve_Local_Stiff(X, theta0[i], rotSprK[i])  # (9,9)
    ...
    def Solve_Global_Theta(self, node, U):
        """
        Update theta_current_vec for all springs
        """
        if self.node_ijk_mat is None:
            raise ValueError("rotSpr3N.node_ijk_mat is not set.")
        rotSprIJK = np.asarray(self.node_ijk_mat, dtype=int)
