...
class Plot_Rolling_Bridge:
    ...
    def Plot_Shape_Spr_Number(self):
        ...
        if getattr(self.assembly, "rot_spr_4N", None) is not None:
            spr = np.array(self.assembly.rot_spr_4N.node_ijkl_mat)
            for i, (_, n2, n3, _) in enumerate(spr):
                p2 = node0[self._to0(n2)]
                p3 = node0[self._to0(n3)]
                p = 0.5 * (p2 + p3)
                ax.text(p[0], p[1], p[2], str(i + 1), color="blue")
        elif getattr(self.assembly, "rot_spr_3N", None) is not None:
            spr = np.array(self.assembly.rot_spr_3N.node_ijk_mat)
            for i, (_, n2, _) in enumerate(spr):
    ...
    def Plot_Shape_Bar_Failure(self, pass_yn):
        ...
        fig, ax = self._setup_ax()
        self._plot_cst_faces(ax, node0, facecolor="yellow", alpha=0.2, edgecolor="k", linewidth=1.0)
