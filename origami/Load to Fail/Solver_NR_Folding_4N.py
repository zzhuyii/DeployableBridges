...
class Solver_NR_Folding_4N:
    ...
    def Solve(self):
        ...
        rot_spr = getattr(assembly, "rot_spr_4N", None)
        if rot_spr is None:
            rot_spr = assembly.rotSpr

        sprZeroStrain_before = rot_spr.theta_current_vec
        ...
        for i in range(increStep):
            ...
            while step < iterMax and R > tol:
                rot_spr.theta_stress_free_vec = sprZeroStrain_current
                ...
                try:
                    ...
                    deltaU = np.linalg.lstsq(K_mod, unbalance_mod, rcond=None)[0]

                for j in range(NodeNum):
                    U[j, :] += deltaU[3*j:3*(j+1)]

                R = np.linalg.norm(deltaU)
                if self.verbose:
