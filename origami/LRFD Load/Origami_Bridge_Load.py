...
def main():
    ...
    g = 9.81
    L_total = 0.0
    for n1, n2 in bar.node_ij_mat:
        L_total += np.linalg.norm(node.coordinates_mat[n1 - 1, :] - node.coordinates_mat[n2 - 1, :])
    ...
    fig_stress.savefig(os.path.join(out_dir, "Origami_Bridge_Load_Bar_Stress.png"), dpi=200, bbox_inches="tight")
    plt.close(fig_stress)

    fig_fail = plots.Plot_Shape_Bar_Failure(pass_yn)
    fig_fail.savefig(os.path.join(out_dir, "Origami_Bridge_Load_Bar_Failure.png"), dpi=200, bbox_inches="tight")
    plt.close(fig_fail)

    fig_def = plots.Plot_Deformed_Shape(U_end)
    fig_def.savefig(os.path.join(out_dir, "Origami_Bridge_Load_Deformed.png"), dpi=200, bbox_inches="tight")
    plt.close(fig_def)
